# git-wmf: A CBOR-based Git Worktree Manifest Format
...
- **1.0.0-draft.2:** Switched serialization to CBOR with canonical encoding per RFC 7049 Section 3.9.
...
- **Schema-First:** The structure is rigid and independent of the data it contains. Keys are static `unicode` literals, aligning with CBOR text string keys, which simplifies parsing into strongly-typed languages.
...
| :----------------- | :----------------- | :------------------------------------------------------------------------------------ |
| `git-wmf-manifest` | **[Section 4]**    | The entire, self-contained `git-wmf` payload, whose root is a CBOR `map`.             |
...
This subsection defines the foundational data types and logical primitives from CBOR and CDDL used to construct the manifest. It maps human-readable terms to their formal CDDL definitions.

| Human Term / Primitive   | Definition and CDDL Mapping                                                                               | CDDL Rule                                                               |
| :----------------------- | :-------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------- |
| `array`                  | A CBOR `array`: an ordered, zero-indexed sequence of values. Example: The `roots` component.              | `[]`                                                                    |
...
| `map`                    | A CBOR `map`: an unordered collection of key-value pairs. Represented by curly braces (`{}`).             | `{}`                                                                    |
...
To ensure deterministic serialization, the entire manifest MUST be encoded using Canonical CBOR as defined in RFC 7049 Section 3.9. This includes using definite lengths, the shortest possible encodings, and sorting map keys by their serialized byte values.
...
The root object of a `git-wmf-manifest` is a single CBOR `map`. This map contains a set of REQUIRED and conditionally present keys that together define the manifest's metadata, directory structure, and file content.
...
The `meta` component is a CBOR `map` for storing contextual information about the manifest.
...
| :----------------------------- | :---------------------------- | :---------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `norm-schema_version`          | `semver_string`               | REQUIRED    | The version of this schema (e.g., `"1.0.0-draft.2"`).                                                                                                                                                              |
...
All CBOR map keys and fields specified as `tstr` (e.g., `git-mode`, `hint-generator_name`) MUST be valid, UTF-8 encoded `unicode`.
...
To achieve deterministic serialization as required by [Section 3.5: Principle of Sorted Components], all `git-wmf` manifests MUST be encoded using Canonical CBOR as defined in RFC 7049 Section 3.9. This includes:

- Using definite-length encodings for arrays, maps, and byte/text strings.
- Using the shortest possible encoding for integers and floating-point numbers.
- Sorting map keys in bytewise lexicographical order based on their serialized forms.
- Avoiding indefinite-length encodings and unnecessary tags.

**Generators** MUST produce canonical encodings. **Consumers** MAY validate canonicity during parsing for strict conformance.
...
This appendix provides a non-trivial but simple manifest example in a human-readable JSON-like format (note that actual manifests are CBOR binary; hashes are hex strings for readability, but would be binary in CBOR; git-name is shown as string but is bstr: e.g., b'hello.txt').
...
{
  "meta": {
    "norm-schema_version": "1.0.0-draft.2",
...
```cddl
;
; git-wmf Specification v1.0.0-draft.2 in CDDL
;
; NAMING POLICY:
...
- Git LFS Specification: https://git-lfs.github.com/spec/v1